...
void connect_to_redis_server(char* redis_host)
{
    ...
    if (!redis_host)
    {
        pton_ret = inet_pton(AF_INET, REDIS_SERVER_HOST, &redis_srvaddr.sin_addr.s_addr);
    }
    else
    {
        pton_ret = inet_pton(AF_INET, redis_host_ip, &redis_srvaddr.sin_addr.s_addr);
    }
...
void enter_server_loop(int server_socket)
{
    ...
    while(1)
    {
        ...
        // A better way to handle accept syscall is to check for EINTR in errno and retry for interrupts
        int client_socket = accept(server_socket, (struct sockaddr *)&client_addr, &client_addr_len);
        if (client_socket == -1) fatal_error("accept()");
        /*
            This avoids the need for handling singal interrupts using SO_RESTART.
            Not every syscall can be restarted, for those syscalls, use the below trick to handle interruption
            while(1)
            {
...
void print_stats(int signo)
{
    ...
    if (getrusage(RUSAGE_SELF, &myusage) < 0) fatal_error("getrusage()");
    if (getrusage(RUSAGE_CHILDREN, &childusage) < 0) fatal_error("getrusage()");
...
{
    /* call wait() to reap as many zombie children so that process accounting get stats */
    while (1)
    {
        int wstatus;
        pid_t pid = waitpid(-1, &wstatus, WNOHANG); // don't block
        if (pid == 0 || pid == -1)
        {
...
int main(int argc, char* argv[])
{
    ...
    // set up signal handler for SIGINT, signal is like a thin wrapper around sigaction with less capability
    signal(SIGINT, print_stats);

    /*
        Setup SIGCHLD handler with SA_RESTART, SIGCHLD signal is sent by kernel to parent when its child exits and becomes a zombie waiting to be reaped
        System calls like accept() or other slow blocking calls will fail with EINTR when interrupted by a signal
    */
    struct sigaction sa;
    memset(&sa, 0, sizeof(sa));
    sa.sa_handler = sigchld_handler;
    sa.sa_flags = SA_RESTART; // retry existing syscalls on interrupt eg: accept()
    sigfillset(&sa.sa_mask);
    sigaction(SIGCHLD, &sa, NULL);

    /*
        Unhandled signals are either ignored by default or they terminate the process. eg: SIGINT
        explicitely telling to ignore SIGCHLD tells kernel to auto reap child processes on parent's behalf
