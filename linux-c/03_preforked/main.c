...
/*
    Create PREFORK_CHILDREN number of processes.
    Each of these processes accepts and serves client requests.
    Can multiple process wait on accept on same port?
    Ans -> Can't call bind() multiple times, but any number of processes that inherit the listening socket,
    can call accept(). OS will equally try to distribute incoming connections among them.

    Thundering Herd problem: Many processes blocked on same event eg: accept()
    Kernel need to wake up processes when event has occurred. If there are many processes,
    Scheduler need to figure out which process or thread will get to run and others will go back to blocking
    this wastes CPU cycles every time an event that potentially turns processes/threads runnable occurs

    This problem is solved in Linux as only 1 process/thread is woken up but this is a pattern
*/
pid_t create_child(int index, int listening_socket)
{
    ...
    pid = fork();
    if (pid < 0) fatal_error("fork()");
    else if (pid > 0) return pid; // parent
